...
void main()
{
    ...
    vec4 color2 = atten  * vec4(globalAmbient + ambient + diffuse + ambient2 + diffuse2, 1.0);

    gl_FragColor = color2 * texture2D( texture, texCoord * texScale) + vec4(specular, 1.0)+ vec4(specular2, 1.0);
}